...
class MeanReversionStrategy(bt.Strategy):
    ...
    - Dynamic monthly capital redistribution across remaining months
    ...
    def next(self) -> None:
        ...
        if self.initial_cash is None:
            ...
            print(
                f"[MeanReversion] INITIALIZATION | Initial cash: ${self.initial_cash:,.2f} | "
        ...
        if signal_ok:
            ...
            print(
                f"[MeanReversion] SIZING CALC | Monthly budget: ${self.current_month_budget:,.2f} | "
                f"Remaining deployable: ${remaining_deployable:,.2f} | "
            ...
            if invest > 0:
                ...
                print(
                    f"[MeanReversion] SIZING CALC | Invest: ${invest:,.2f} / Close: ${close:.2f} = Size: {size:.6f} shares"
                )
    ...
    def notify_order(self, order: bt.Order) -> None:
        ...
        if order.status == order.Completed:
            if order.isbuy():
                ...
                print(
                    f"[MeanReversion] ORDER EXECUTED | Price: ${entry_price:.2f} | "
                ...
                print(
                    f"[MeanReversion] DEPLOYMENT UPDATE | Total invested so far: ${self.total_invested:,.2f} | "
    ...
    def stop(self) -> None:
        ...
        plt.title(
            f"MeanReversion (Dynamic Monthly Redistribution) - Portfolio Breakdown ({num_entries_text})"
        )
        plt.legend()
        plt.grid(True)
