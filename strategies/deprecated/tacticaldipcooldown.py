...
class TacticalTrendDipCooldown(bt.Strategy):
    """
    ...
    - COOLDOWN: Wait 4 weeks (28 days) after any buy before next purchase.
    ...
    params = dict(
        ...
        cooldown_days=28,  # 4 weeks = 28 days
    ...
    def __init__(self):
        ...
        self.last_buy_date = None  # Track last buy for cooldown
    ...
    def start(self):
        ...
        self.buy_count = 0
        print(f"[TacticalDipCooldown] start() called - initial_cash captured: ${self.initial_cash:,.2f}")
    ...
    def _buy_cash_amount(self, cash_to_use: float):
        ...
        if self.last_buy_date is not None:
            ...
            if days_since_last_buy < self.p.cooldown_days:
                ...
                if self.buy_count <= 5:  # Log cooldown rejections for first few attempts
                    print(f"[TacticalDipCooldown] Cooldown active on {current_date}: {days_since_last_buy} days since last buy (need {self.p.cooldown_days})")
                return

        size = int(cash_to_use / self.close[0])
        if size > 0:
            # Calculate days since last buy BEFORE updating
            if self.last_buy_date is None:
                days_since = "N/A (first buy)"
            else:
                days_since = (current_date - self.last_buy_date).days

            self.buy(size=size)
            self.buy_count += 1

            if self.buy_count <= 5:  # Log first 5 buys only
                print(f"[TacticalDipCooldown] Buy #{self.buy_count} on {current_date}: size={size}, price=${self.close[0]:.2f}, invest=${cash_to_use:.2f}, days_since_last={days_since}")

            # Update last buy date AFTER logging
    ...
    def stop(self) -> None:
        ...
        print(f"[TacticalDipCooldown] stop() called - total_buys={self.buy_count}, final_value=${final_value:,.2f}, final_cash=${final_cash:,.2f}, position_size={position.size}")
        ...
        plt.xlabel("Date")
        plt.ylabel("Value ($)")
        plt.title("TacticalDipCooldown (4-Week Cooldown) - Portfolio Breakdown")
        plt.legend()
        plt.grid(True)
        plt.tight_layout()
